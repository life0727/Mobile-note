<template>
  ...
  trigger="click">
    <div style="height: 250px;width:100%;">
      ...
      <p style="width:90%;position: absolute;top: 80px;left: 15px;font-size: 16px;">当前项目：<a href="javascript:;" @click="Tab_card(duibiData[0].id,duibiData[0].name)" style="color: #00b38a">{{duibiData[0].name}}</a></p>
    ...
    border: 1px solid #e4e4e4;border-radius: 4px;">
          ...
          <router-link to="/keyword">
            <el-button icon="el-icon-plus" type="text" style="margin-left: 55px;color: rgb(0, 179, 138)" @click="$store.state.keyword_tab = '_competTab'">添加竞品</el-button>
          </router-link>  
        </div>
      ...
      <el-button style="padding: 4px 5px;font-size: 14px;background-color: rgb(0, 179, 138);border-color:rgb(0, 179, 138);color:white;position: absolute;bottom: 10px;right: 100px;"  @click="save">导出</el-button>
  ...
  <div class="block"  >
    <div class="filter_event" style="width: 100%;border: 1px solid rgb(228,228,228);background-color:rgb(247,247,247);position:relative;height: 48px;">
      ...
      <el-popover
        v-loading.fullscreen.lock="loading_start" element-loading-text="系统拼命加载中" element-loading-spinner="el-icon-loading"
        placement="bottom"
        @hide="dom_search"
        width="450"
      ...
      <span class="search_start" style="line-height: 45px;color: #00b38a;cursor: pointer;margin-left: 30px;" id="search" @click="_search" >搜索</span>
    </div>
  </div>

 <div class="Media container" style="background-color: #fff;padding-bottom:220px;position: relative;border: 1px solid rgba(228, 228, 228, 1);border-top-width: 0;">  
  <p style="padding: 10px 0 10px 5px;color:#999999;margin: 0">当前项目：<b style="color: #00b38a;margin-right:20px;">{{current_project_dom_name}}</b>已选媒体分类：<span style="color: #00b38a;">{{domain_arr.length == 0 ? '全部' : domain_arr.toString()}}</span></p>
   ...
   <div id="content" style="height: 500px;">
      <div style="width: 400px;height: 50px;border-bottom: 1px solid #ebebeb;border-top: 1px solid #ebebeb;line-height: 50px;float: left;display: inline-block">
        ...
        <span style="margin-left: 100px">count</span>
      </div>
      ...
      <el-checkbox style="margin: 10px 10px 10px 15px;top: 385px;" v-model="select_All"   v-show="this.data.length!==0" >全选所有</el-checkbox>
      <el-button type="text" @click="remove" style="color:#f56c6c;margin-left: 5px;position: absolute;top:485px" v-show="(select_All&&this.data.length!==0)||this.Selection.length!==0" >确认删除</el-button>
      <el-pagination
            class="page"
        ...
        <el-checkbox style="margin: 10px 10px 10px 15px;top:400px;left:416px" v-model="select_All_per"   v-show="this.per_data.length!==0&&tabShow == 0&&this.data.length > 0" >全选所有</el-checkbox>
        <el-button type="text" @click="del_('perlist_selection','perList','mention','per_data')" style="color:#f56c6c;margin-left: 5px;position: absolute;top:500px;left:625px" v-show="(select_All_per&&this.per_data.length!==0)||this.perlist_selection.length!==0" >确认删除</el-button>
        ...
        <el-checkbox style="margin: 10px 10px 10px 15px;top:400px;left:642px" v-model="select_All_key"   v-show="this.key_data.length!==0&&tabShow == 0&&this.data.length > 0" >全选所有</el-checkbox>
        <el-button type="text" @click="del_('keywordlist_selection','keywordList','mention','key_data')" style="color:#f56c6c;margin-left: 5px;position: absolute;top:500px;left:960px" v-show="(select_All_key&&this.key_data.length!==0)||this.keywordlist_selection.length!==0" >确认删除</el-button>
        ...
        <el-checkbox style="margin: 10px 10px 10px 15px;top:405px;left:416px" v-model="select_All_article"   v-show="this.article_data.length!==0&&tabShow == 1&&this.data.length > 0" >全选</el-checkbox>
        <el-button type="text" @click="del_('articlelist_selection','eventArticleList','id','article_data','tabledata_articlelist_list')" style="color:#f56c6c;position: absolute;top:505px;left:625px" v-show="(select_All_article&&this.article_data.length!==0)||this.articlelist_selection.length!==0" >确认删除</el-button>
        <el-pagination
              v-if="pageShow"
              class="page"
     ...
     <!-- 第二级模态框 -->
      <el-dialog title="相关新闻" :visible.sync="dialo_media" width="700px"  id="dialog_list_media">
        <div style="position:absolute;left: 35px;padding: 10px;width:620px;height: 520px;">
            ...
            <el-button v-show="this.second_selection.length != 0"  type="text" style="position: absolute;left: 90px; bottom: 6px;padding: 7px 3px;color:#999999;" @click="del_2('second_selection','id')">确认删除</el-button>
            <el-pagination
                  v-if="pageShow"
                  class="page"
...
export default {
  ...
  methods: {
    ...
    search(duibiTable_flag,Id){// duibiTable_flag false 则为点击的自身项目下的媒体, true 为点击竞品项目下的媒体
        ...
        publicSearch('rsa/project/'+project_id+'/media',"GET",params).then((data) =>{//ajax
          ...
          if(successBack(data,this)){
            ...
            data.data.mediaList.sort(_Sort('count'));
    ...
    rowClick(dta){//dom遍历等
      console.log(dta)
      this.currentmediaName = dta.name.slice(0,dta.name.indexOf('/'));
